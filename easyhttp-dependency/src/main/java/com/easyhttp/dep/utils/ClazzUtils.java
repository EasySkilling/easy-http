...
public class ClazzUtils {
    ...
    /**
     * 判断child是否father的子类或者同一类型
     * @param types
     * @param child
     * @param father
     * @return
     */
    public static boolean isSubOrSameClazz(Elements elements, Types types, Messager messager, TypeMirror child, Class<?> father) {
        // 对比的时候两个类可能会存在泛型，这时比较就不会相等，因此要忽略泛型获取通用类名然后比较
        TypeElement fatherElement = elements.getTypeElement(father.getCanonicalName());
    ...
    public static boolean isSubOrSameClazz(Elements elements, Types types, Messager messager, TypeMirror child, String fatherFullName) {
        // 对比的时候两个类可能会存在泛型，这时比较就不会相等，因此要忽略泛型获取通用类名然后比较
        TypeElement fatherElement = elements.getTypeElement(fatherFullName);
        TypeElement childElement = (TypeElement) types.asElement(child);
        if (childElement == null) {
            return false;
        }
...
//        messager.printMessage(Diagnostic.Kind.NOTE, "outerClazzFullName = " + outerClazzFullName);
        ...
        if (left != -1 && right != -1) {
            ...
            messager.printMessage(Diagnostic.Kind.NOTE, "innerClazzName = " + innerClazzName);
            messager.printMessage(Diagnostic.Kind.NOTE, "outerClazzName = " + outerClazzName);
